 ...
 */
define("seajs/plugins/seajs-debug/1.1.1/seajs-debug-debug", [ "./panel-debug", "./store-debug", "./config-debug", "./panel-debug.css" ], function(require, exports, module) {
    var doc = document, loc = location;
    var debugPanel = require("./panel-debug"), config = debugPanel.config;
        ...
        seajs.find = function(selector) {
            ...
            for (var uri in cachedModules) {
                if (cachedModules.hasOwnProperty(uri)) {
                    if (typeof selector === "string" && uri.indexOf(selector) > -1 || selector instanceof RegExp && selector.test(uri)) {
...
// Debug Panel Object
define("seajs/plugins/seajs-debug/1.1.1/panel-debug", [ "seajs/plugins/seajs-debug/1.1.1/store-debug", "seajs/plugins/seajs-debug/1.1.1/config-debug" ], function(require, exports, module) {
    var doc = document, loc = location, global = this;
    var store = require("seajs/plugins/seajs-debug/1.1.1/store-debug");
    var config = require("seajs/plugins/seajs-debug/1.1.1/config-debug");
    var MAX_TRY = 100;
    var pollCount = 0;
    ...
    require("seajs/plugins/seajs-debug/1.1.1/panel-debug.css");
    function DebugPanel() {
        this.element = null;
    ...
    DebugPanel.prototype._renderMap = function() {
        ...
        for (var i = 0; i < config.mapping.length; i++) {
            ...
            tmpHTML += "<p>" + '<input type="text" placeholder="Input source URI" title="Source URI" value="' + item[0] + '" />' + '<button style="cursor: default;">&#xe80c;</button>' + '<input type="text" placeholder="Input target URI" title="Target URI" value="' + item[1] + '" />' + '<button data-name="add" ' + (item[2] ? "" : 'style="display: none;"') + ">&#xe804;</button>" + '<button data-name="red" ' + (item[2] ? 'style="display: none;"' : "") + ">&#xe805;</button>" + "</p>";
        }
        return '<div id="' + PREFIX + 'map" style="display: none;">' + tmpHTML + "</div>";
    };
    // Editor
    ...
    DebugPanel.prototype._renderStatus = function() {
        ...
        for (var i = 0; i < this.statusInfo.length; i++) {
            ...
            var status = item[0];
            tmpHTML += "<button " + (config[status] ? 'class="' + STATUS_BUTTON_ON_CLS + '"' : "") + ' title="' + item[config[status] ? 1 : 2] + '">' + item[3] + "</button>";
        }
        return '<div id="' + PREFIX + 'status" style="display: none;">' + tmpHTML + "<span></span>" + "</div>";
    };
    // Main button bar
    DebugPanel.prototype._renderAction = function() {
        this.actionInfo = [ // [Button Text, click callback fn, remove ?seajs-debug or not]
        [ "Save", function() {
            var mappingString = [];
        ...
        return '<div id="' + PREFIX + 'action" style="display: none;">' + tmpHTML + "</div>";
    };
    // Meta button bar
    DebugPanel.prototype._renderMeta = function() {
        this.metaInfo = [ // [show or hide, icon, title, click callback fn]
        ...
        for (var i = 0; i < this.metaInfo.length; i++) {
            ...
            tmpHTML += '<button title="' + item[2] + '">' + item[1] + "</button>";
        }
        return '<div id="' + PREFIX + 'meta">' + tmpHTML + "</div>";
    };
    // Health DIV
    DebugPanel.prototype._renderHealth = function() {
        return '<div id="' + PREFIX + 'health" style="display: none;">' + "</div>";
    };
    // Bind mapping area
...
define("seajs/plugins/seajs-debug/1.1.1/store-debug", [], function(require, exports, module) {
...
define("seajs/plugins/seajs-debug/1.1.1/config-debug", [ "seajs/plugins/seajs-debug/1.1.1/store-debug" ], function(require, exports, module) {
    var store = require("seajs/plugins/seajs-debug/1.1.1/store-debug");
    // Main config
    var config = {
